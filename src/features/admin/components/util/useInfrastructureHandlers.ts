...
export function useInfrastructureHandlers(facilityId: string) {
    ...
    const [selectedInfrastructure, setSelectedInfrastructure] = useState<InventoryItem | null>(null);
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            toast.error(
                error?.response?.data?.message || "Failed to delete infrastructure. Please try again.",
                { duration: 5000 }
            );
        }
    };
