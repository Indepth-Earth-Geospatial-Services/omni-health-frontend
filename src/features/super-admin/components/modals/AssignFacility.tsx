...
export default function AssignFacilityModal({
  ...
  useEffect(() => {
    ...
    if (isListOpen) document.addEventListener("mousedown", handleOutside);
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white shadow-2xl">
        {/* Header band */}
        <div className="from-primary to-primary/80 relative overflow-hidden bg-gradient-to-r px-6 py-5">
        ...
        <div className="max-h-[calc(100vh-120px)] overflow-y-auto p-6">
          ...
          <div className="mb-5 flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3.5">
            <div className="from-primary to-primary/70 flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br text-sm font-bold text-white shadow-sm">
            ...
            <span
              className={`shrink-0 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${getRoleBadge(user.role)}`}
            >
              {user.role.replace("_", " ")}
            </span>
          </div>

          {/* Currently assigned LGAs (derived from managed_facilities) */}
          ...
          <div className="mb-4">
            ...
            <div ref={listRef} className="relative">
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-500 transition-colors hover:border-slate-400 focus:outline-none"
              >
                <span>
                      ...
                      filteredLgas.map((lga) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <MapPin
                              size={13}
                              className={checked ? "text-primary" : "text-slate-400"}
                            />
                            <span
                              className={
                                checked
