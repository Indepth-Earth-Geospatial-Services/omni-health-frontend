...
async function getStaff(facilityId: string): Promise<Staff[]> {
    const response = await apiClient.get(`/admin/staff/${facilityId}`);
    return response.data;
}

async function createStaff(facilityId: string, staffData: any): Promise<Staff> {
    const response = await apiClient.post(`/admin/facility/${facilityId}/staff`, staffData);
    return response.data;
}

async function updateStaff(staffId: string, staffData: any): Promise<Staff> {
